...
import com.example.littletreemusic.util.LyricUtil;
...
public class LyricView extends View {
    ...
    public void setLrc(String lrc) {
        list = LyricUtil.parseStr2List(lrc);
        invalidate();
    }
    ...
    public LyricView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        highLineColor = ta.getColor(R.styleable.LrcView_hignLineColor, getResources().getColor(R.color.绿色));
        lrcColor = ta.getColor(R.styleable.LrcView_lrcColor, getResources().getColor(R.color.白色));
...
//        drawLrc1(canvas);
    ...
    private void drawLrc2(Canvas canvas, int currentMillis) {
        if (mode == 0) {
            ...
            Lyric Lyric = list.get(currentPosition);
            long start = Lyric.getStart();
            long end = Lyric.getEnd();
